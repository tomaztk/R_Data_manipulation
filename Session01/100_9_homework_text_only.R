...
# 1. Create a variable num_var with a numeric value, char_var 
#    with a character string, and bool_var with a logical (TRUE/FALSE) value.
# 2. Use the class() function to check the data types of each 
#    variable (num_var, char_var, bool_var).
# 3. Convert num_var to character and store it in a new variable num_char.
# 4. Print the data type of num_char to confirm the conversion. 
#   And check the class() function for the num_char variable.
...
# 1. Create a numeric vector num_vec with values 
#     2, 4, 6, 8, 10. Create this vector in two different ways.
# 2. Access the third element in num_vec and assign
#     it to a variable third_elem.
# 3. Calculate the sum of all elements in num_vec 
#    using the sum() function. 
#  Extra points: write your own function using loop
# 4. Add a new element 12 to num_vec using c() 
#    and print the updated vector.

num_vec <- c(2, 4, 6, 8, 10)
num_vec2 <- seq(from = 2, to = 10,  by = 2)
num_vec2A <- seq(2,10,2)
num_vec2A <- seq(2,   10,    2)

element1 <- 12
num_vec <- c(num_vec, element1)

sum(num_vec, element1)


sum_var <- 0

for(i in 1:length(num_vec)){
  sum_var <- sum_var + num_vec[i]
  }
...
# 1. Create a 3x3 matrix mat with values from 1 to 9 
# using the matrix() function and orientation by rows.
# 2. Calculate the row-wise sum and column-wise sum of
#    mat using rowSums() and colSums(). 
#     Extra points: write your own function for sum of
#     columns or rows.
...
matrika <- matrix(1:9, nrow=3, ncol=3, byrow=TRUE)
...
#Access the element in the second row 
#and third column and store it in matrix_el23.

matrix_el23 <- matrika[2,3]
print(matrix_el23)

update_matrika <- matrika*2
print(update_matrika)


update_matrika<- matrika[2,3] * 2
print(update_matrika)

dim(matrika)

# multiply  position 2_3 in 3x3 matrix with value 2
for (i in 1:3) {
  for (j in 1:4){
    print (i)  
  }
...
# Hint: total of 5 rows and 3 columns. and use two functions: data.frame() and c()
...
df_subset <- df[  , c("Ime","Kredit_sposob") ]

df_subset

# 4. Extract and print the value from 
# original df from column  'Starost' of the third person.

df[ 3 , c("Starost") ]

df[ df$Ime=="Tomaz" , c("Starost") ]

# Select all the dows where 'Kredit_sposob' is equal
# or bigger than 0.52  and the name ('Ime') starts with "Ti" and 
#   return only  columns 'Ime' and 'Starost' 
# and create a new dataframe named df_subset2

help("substr")
# substr(x, start, stop)

df_subset2 <- df[ df$Kredit_sposob >= 0.52 & substr(df$Ime, 1,2) == "Ti", 
                  c("Ime","Starost") ]
...
# ======================================
# Task 13: The grande finale:  
# Data Frames, Loops, and Custom Sorting
# ======================================
...
#     Hint: you can use function tapply() or use data type list() and function mean()
